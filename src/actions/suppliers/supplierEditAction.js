...
export const fetchData = (idFornecedor) => async (dispatch) => {
    ...
    try {
      const response = await axios.get(`http://192.168.0.15:8080/fornecedor/${idFornecedor}`);
...
export const editFornecedor = (body, idFornecedor) => async (dispatch, getState) => {
    ...
    try {
        const response = await axios.put(`http://192.168.0.15:8080/fornecedor/${idFornecedor}`, body);
        if (response.status == 200) {
